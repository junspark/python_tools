# -*- coding: utf-8 -*-
'''
*Module G2img_1ID_32bit_TIFsum.py: 1ID normalized 32bit Pixirad TIF for rapid access measurement*
--------------------------------------------------
...
class TIF_ReaderClass(G2obj.ImportImage):
    ...
    def __init__(self):
        super(self.__class__,self).__init__( # fancy way to self-reference
            ...
            formatName = '1ID normalized 32bit Pixirad TIF',
            longFormatName = '1ID normalized 32bit Pixirad TIF generated by Matlab batch summation script'
            )
...
def GetTifData(filename):
    '''Confirm that the file is in fact a 32bit tiff image with the correct
    ...
    tiff images into the 32bit format with a different buffer in order to hold
    ...
    if IFD[258][2][0] == 32:                                                    #summed files are 32 bit to hold the required amount of data
        print("We are here %d" % (IFD[258][2][0]))
        if sizexy == [1024, 402] or sizexy == [402, 1024]:                      #confirms that it has the proper size
            tifType = '1ID summed 32bit Pixirad'
            pixy = [62.,62.]                                                      #sets the pixel size
            print ('Read 1ID normalized 32bit Pixirad tiff file: '+filename)
            ...
            image = np.frombuffer(File.read(4*Npix),dtype=np.float32)
    ...
    if sizexy[1]*sizexy[0] != image.size: # test if resize is allowed
        print("image.size actual %d %d" % (sizexy[1], sizexy[0]))
